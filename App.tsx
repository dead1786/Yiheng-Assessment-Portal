      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100">
            ...
            <p className="text-gray-500 mb-6 text-sm">程式執行時遇到了預期外的狀況。<br /><span className="text-xs text-red-400 mt-2 block bg-red-50 p-2 rounded break-all">{this.state.error?.message}</span></p>
            <button onClick={() => { localStorage.removeItem('app_session'); window.location.reload(); }} className="w-full py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 font-bold flex items-center justify-center"><RefreshCw size={18} className="mr-2" /> 清除暫存並重整</button>
          </div>
        </div>
      );
    }
...
const App: React.FC = () => {
  ...
  const [view, setView] = useState<'dashboard' | 'form' | 'history' | 'profile' | 'schedule' | 'full-schedule' | 'report-deficiency' | 'clock-in'>('dashboard');
  ...
  const performSync = useCallback(async () => {
    ...
    try {
        ...
        // 這裡會呼叫後端的 checkLoginStatus，現在它會回傳最新的 userDetails
  ...
  const handleManualRefresh = async () => {
      setIsSyncing(true);
      await performSync();
      setTimeout(() => setIsSyncing(false), 800); // 視覺延遲
  };
  ...
  const renderContent = () => {
    ...
    switch (view) {
      ...
      case 'clock-in': return <ClockInForm user={user} apiUrl={apiUrl} onBack={forceToDashboard} onAlert={showAlert} />;
